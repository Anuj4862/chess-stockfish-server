...
class UCIEngine:
    ...
    def analyze(self, moves, movetime, depth=0, multipv=1):
        ...
        with self._lock:
            ...
            best_cp, best_mate, best_move, depth_reached, best_pv = 0, None, '', 0, ''
                ...
                if line.startswith('info') and is_mpv1:
                    if 'pv' in parts:
                        ...
                        if pv_idx + 1 < len(parts):
                            best_pv = ' '.join(parts[pv_idx + 1:pv_idx + 6])  # first 5 moves of PV
                    if 'depth' in parts:
            ...
            return {'cp': best_cp, 'mate': best_mate, 'move': best_move, 'depth': depth_reached, 'pv': best_pv}
...
def analyze_game():
    ...
    for i in range(len(all_moves) + 1):
        ...
        best_pv   = sf.get('pv', '')   # principal variation from SF
        ...
        r = {
            ...
            'best_move': best,
            'best_move_pv': best_pv,   # SF's principal variation (UCI) for this position
